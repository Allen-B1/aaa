...
def parse_note(elem: ET.Element, divisions: float) -> Note:
    ...
    alter = None if pitch_elem.find("alter") is None else pitch_elem.find("alter").text
    pitch_name = step + ('#' if alter == "1" else 'b' if alter == "-1" else "") + octave
    pitch = pretty_midi.note_name_to_number(pitch_name)

    duration_int = int(elem.find("duration").text)
...
def parse_measure(elem: ET.Element, divisions: int) -> Tuple[Measure, int]:
    divisions = int(divisions if elem.find("attributes/divisions") is None else elem.find("attributes/divisions").text)
    ...
    for child_elem in elem:
        ...
        if child_elem.tag == "note":
            ...
            duration_int = int(child_elem.find("duration").text)
            prev_duration = duration_int / divisions
